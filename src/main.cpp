...
int64 nChainStartTime = 1375341341;
...
const string strMessageMagic = "GrowthCoin Signed Message:\n";
...
int64 GetProofOfWorkReward(int nHeight, int64 nFees, uint256 prevHash)
{
	...
	if(rand > 5000 && rand < 5101)		
	{
		cseed_str = prevHash.ToString().substr(10,7);
		cseed = cseed_str.c_str();
		seed = hex2long(cseed);
		rand = generateMTRandom(seed, 501);
		nSubsidy = (499 + rand) * COIN;
	}

	// Subsidy is cut in half every 172,800 blocks, which will occur approximately every 3 months
	nSubsidy >>= (nHeight / 172800); 
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex, bool fJustCheck)
{
    ...
    bool fEnforceBIP30 = true; // Always active in GrowthCoin
    bool fStrictPayToScriptHash = true; // Always active in GrowthCoin
...
bool CheckDiskSpace(uint64 nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "GrowthCoin", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
string GetWarnings(string strFor)
{
    ...
    if (Checkpoints::IsSyncCheckpointTooOld(60 * 60 * 24 * 400) && !fTestNet && !IsInitialBlockDownload())
    {
        nPriority = 100;
...
unsigned char pchMessageStart[4] = { 0xa5, 0xef, 0xdb, 0xfd };
